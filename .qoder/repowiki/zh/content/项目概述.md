# 项目概述

<cite>
**本文档中引用的文件**
- [README.md](file://README.md)
- [rdagent/core/evolving_agent.py](file://rdagent/core/evolving_agent.py)
- [rdagent/core/evolving_framework.py](file://rdagent/core/evolving_framework.py)
- [rdagent/components/coder/CoSTEER/__init__.py](file://rdagent/components/coder/CoSTEER/__init__.py)
- [rdagent/components/coder/CoSTEER/evolving_strategy.py](file://rdagent/components/coder/CoSTEER/evolving_strategy.py)
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py)
- [rdagent/app/cli.py](file://rdagent/app/cli.py)
- [rdagent/scenarios/data_science/loop.py](file://rdagent/scenarios/data_science/loop.py)
- [rdagent/app/qlib_rd_loop/quant.py](file://rdagent/app/qlib_rd_loop/quant.py)
- [rdagent/core/conf.py](file://rdagent/core/conf.py)
- [rdagent/components/coder/CoSTEER/config.py](file://rdagent/components/coder/CoSTEER/config.py)
- [rdagent/components/agent/context7/__init__.py](file://rdagent/components/agent/context7/__init__.py)
- [rdagent/components/agent/context7/conf.py](file://rdagent/components/agent/context7/conf.py)
- [rdagent/components/agent/mcp/__init__.py](file://rdagent/components/agent/mcp/__init__.py)
- [rdagent/components/agent/base.py](file://rdagent/components/agent/base.py)
- [rdagent/app/finetune/data_science/scen.py](file://rdagent/app/finetune/data_science/scen.py)
- [rdagent/app/finetune/llm/scen.py](file://rdagent/app/finetune/llm/scen.py)
- [rdagent/app/finetune/data_science/conf.py](file://rdagent/app/finetune/data_science/conf.py)
- [rdagent/app/finetune/llm/conf.py](file://rdagent/app/finetune/llm/conf.py)
- [rdagent/app/data_science/conf.py](file://rdagent/app/data_science/conf.py)
</cite>

## 更新摘要
**已做更改**
- 新增LLM微调功能描述，涵盖数据科学和LLM场景的微调流程
- 增加MCP集成机制说明，重点描述工具调用架构
- 引入Context7智能文档管理功能，支持基于错误信息的自动化文档检索
- 更新数据科学代理配置选项，增强可配置性与灵活性

## 目录
1. [项目简介](#项目简介)
2. [核心使命与价值](#核心使命与价值)
3. [技术架构概览](#技术架构概览)
4. [核心组件详解](#核心组件详解)
5. [应用场景](#应用场景)
6. [创新特性](#创新特性)
7. [与其他工具的对比](#与其他工具的对比)
8. [总结](#总结)

## 项目简介

RD-Agent是一个基于大语言模型（LLM）驱动的自动化研发框架，旨在实现数据驱动的研发过程自动化。该项目由微软开发，代表了人工智能在科学研究和工程开发领域的重大突破。

### 核心特点

- **自动化研发**：自动化的研究与开发流程，从想法提出到代码实现
- **多场景支持**：涵盖数据科学、量化金融、Kaggle竞赛等多个领域
- **智能进化**：基于多轮迭代、反馈学习和知识增强的自主进化能力
- **LLM驱动**：充分利用大型语言模型的强大推理和生成能力

## 核心使命与价值

### 使命宣言

RD-Agent致力于自动化工业研发过程中最关键和最有价值的部分，专注于数据驱动场景下的模型和数据开发。

### 核心价值主张

1. **知识提取**：从真实世界材料（报告、论文等）中提取关键公式、特征和模型
2. **代码实现**：将提取的知识转化为可运行的代码
3. **创新提案**：基于当前知识和观察提出新想法

### 技术愿景

项目相信，高质量解决方案的关键在于能够像人类专家一样持续改进研发技能的能力。通过不断学习和进化，代理应该能够在各个领域实现卓越的表现。

**章节来源**
- [README.md](file://README.md#L1-L100)

## 技术架构概览

RD-Agent采用模块化架构设计，核心围绕RAGEvoAgent和CoSTEER策略构建。

```mermaid
graph TB
subgraph "用户接口层"
CLI[命令行界面]
WebUI[Web用户界面]
Docker[Docker容器化]
end
subgraph "核心引擎层"
RAGEvoAgent[RAGEvoAgent<br/>进化代理]
CoSTEER[CoSTEER策略<br/>协同进化策略]
EvolvingFramework[进化框架]
end
subgraph "组件库层"
DataScience[数据科学组件]
QuantFinance[量化金融组件]
Kaggle[Kaggle竞赛组件]
GeneralModel[通用模型组件]
end
subgraph "知识管理层"
RAGStrategy[RAG策略]
KnowledgeBase[知识库]
GraphDB[图数据库]
end
subgraph "场景应用层"
MLEBench[MLE-bench基准测试]
QuantTrading[量化交易]
DataMining[数据挖掘]
ResearchCopilot[研究助手]
end
CLI --> RAGEvoAgent
WebUI --> RAGEvoAgent
Docker --> RAGEvoAgent
RAGEvoAgent --> CoSTEER
RAGEvoAgent --> EvolvingFramework
CoSTEER --> RAGStrategy
RAGStrategy --> KnowledgeBase
RAGStrategy --> GraphDB
DataScience --> RAGEvoAgent
QuantFinance --> RAGEvoAgent
Kaggle --> RAGEvoAgent
GeneralModel --> RAGEvoAgent
MLEBench --> DataScience
QuantTrading --> QuantFinance
DataMining --> DataScience
ResearchCopilot --> GeneralModel
```

**图表来源**
- [rdagent/core/evolving_agent.py](file://rdagent/core/evolving_agent.py#L1-L50)
- [rdagent/components/coder/CoSTEER/__init__.py](file://rdagent/components/coder/CoSTEER/__init__.py#L1-L50)
- [rdagent/app/cli.py](file://rdagent/app/cli.py#L1-L30)

### 架构层次说明

1. **用户接口层**：提供多种交互方式，包括命令行、Web界面和容器化部署
2. **核心引擎层**：包含RAGEvoAgent和CoSTEER策略，负责智能决策和进化控制
3. **组件库层**：针对不同场景的专业化组件集合
4. **知识管理层**：支持检索增强生成（RAG）的知识管理系统
5. **场景应用层**：具体的应用场景和基准测试

**章节来源**
- [rdagent/core/evolving_agent.py](file://rdagent/core/evolving_agent.py#L1-L116)
- [rdagent/components/coder/CoSTEER/__init__.py](file://rdagent/components/coder/CoSTEER/__init__.py#L1-L177)

## 核心组件详解

### RAGEvoAgent：进化代理核心

RAGEvoAgent是RD-Agent的核心组件，实现了基于知识增强的进化代理功能。

```mermaid
classDiagram
class RAGEvoAgent {
+max_loop : int
+rag : Any
+evolving_trace : list
+with_knowledge : bool
+with_feedback : bool
+knowledge_self_gen : bool
+multistep_evolve(evo, eva) Generator
}
class EvolvingStrategy {
+scen : Scenario
+evolve() EvolvableSubjects
}
class RAGEvaluator {
+evaluate(eo, queried_knowledge) Feedback
}
class EvoStep {
+evolvable_subjects : EvolvableSubjects
+queried_knowledge : QueriedKnowledge
+feedback : Feedback
}
RAGEvoAgent --> EvolvingStrategy : 使用
RAGEvoAgent --> RAGEvaluator : 使用
RAGEvoAgent --> EvoStep : 创建
```

**图表来源**
- [rdagent/core/evolving_agent.py](file://rdagent/core/evolving_agent.py#L20-L80)

#### 核心工作流程

1. **知识查询**：通过RAG系统查询相关知识
2. **进化策略**：应用CoSTEER策略进行任务进化
3. **评估反馈**：收集执行结果和性能反馈
4. **知识更新**：自动生成和更新知识库
5. **循环迭代**：重复上述过程直到完成

**章节来源**
- [rdagent/core/evolving_agent.py](file://rdagent/core/evolving_agent.py#L60-L116)

### CoSTEER策略：协同进化策略

CoSTEER（Collaborative STEpping Evolutionary Research）是RD-Agent的核心进化策略，实现了多进程协同的任务执行和知识管理。

```mermaid
sequenceDiagram
participant User as 用户
participant CoSTEER as CoSTEER策略
participant RAG as 知识库
participant Evaluator as 评估器
participant Worker as 工作进程
User->>CoSTEER : 提交实验请求
CoSTEER->>RAG : 查询相关知识
RAG-->>CoSTEER : 返回知识片段
CoSTEER->>Worker : 分配任务到多个进程
Worker->>Worker : 并行执行任务
Worker-->>CoSTEER : 返回执行结果
CoSTEER->>Evaluator : 评估执行效果
Evaluator-->>CoSTEER : 返回反馈
CoSTEER->>RAG : 更新知识库
CoSTEER-->>User : 返回最终结果
```

**图表来源**
- [rdagent/components/coder/CoSTEER/evolving_strategy.py](file://rdagent/components/coder/CoSTEER/evolving_strategy.py#L20-L80)
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py#L100-L200)

#### 策略特性

1. **多进程并行**：支持多个任务的并行执行
2. **智能分配**：根据历史反馈智能分配任务
3. **知识融合**：整合成功经验和失败教训
4. **动态调整**：根据执行情况动态调整策略

**章节来源**
- [rdagent/components/coder/CoSTEER/evolving_strategy.py](file://rdagent/components/coder/CoSTEER/evolving_strategy.py#L1-L135)
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py#L1-L200)

### 知识管理系统

RD-Agent采用先进的RAG（Retrieval-Augmented Generation）策略，构建了多层次的知识管理体系。

```mermaid
graph LR
subgraph "知识查询层"
FormerTrace[历史轨迹查询]
Component[组件分析查询]
Error[错误分析查询]
end
subgraph "知识存储层"
SuccessKB[成功知识库]
WorkingKB[工作轨迹库]
ErrorKB[错误知识库]
GraphDB[图数据库]
end
subgraph "知识生成层"
Embedding[向量嵌入]
Clustering[聚类分析]
Summarization[知识摘要]
end
FormerTrace --> SuccessKB
Component --> GraphDB
Error --> ErrorKB
SuccessKB --> Embedding
WorkingKB --> Clustering
ErrorKB --> Summarization
```

**图表来源**
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py#L300-L500)

**章节来源**
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py#L500-L799)

### MCP集成：模块化能力平台

MCP（Modular Capability Platform）是RD-Agent的模块化能力集成框架，支持外部工具和服务的无缝接入。

**章节来源**
- [rdagent/components/agent/mcp/__init__.py](file://rdagent/components/agent/mcp/__init__.py#L1-L11)
- [rdagent/components/agent/base.py](file://rdagent/components/agent/base.py#L135-L156)

#### 架构设计

MCP采用RESTful API架构，允许将任意功能封装为独立的服务模块。每个MCP服务器提供标准化的接口定义和健康检查机制。

```mermaid
graph TD
A[RD-Agent主系统] --> B[MCP代理]
B --> C[Context7文档服务]
B --> D[代码分析服务]
B --> E[模型推理服务]
C --> F[(本地文档库)]
D --> G[(代码知识图谱)]
E --> H[(预训练模型)]
```

**图表来源**
- [rdagent/components/agent/mcp/__init__.py](file://rdagent/components/agent/mcp/__init__.py#L1-L11)

#### 核心优势

1. **松耦合架构**：各服务独立部署和升级
2. **动态注册**：支持运行时动态加载新能力
3. **统一接口**：所有服务遵循标准化调用协议
4. **容错机制**：内置健康检查和降级策略

### Context7：智能文档管理系统

Context7是RD-Agent集成的智能文档管理服务，专注于解决代码开发中的API文档缺失问题。

**章节来源**
- [rdagent/components/agent/context7/__init__.py](file://rdagent/components/agent/context7/__init__.py#L1-L60)
- [rdagent/components/agent/context7/conf.py](file://rdagent/components/agent/context7/conf.py#L1-L32)
- [rdagent/components/coder/data_science/pipeline/eval.py](file://rdagent/components/coder/data_science/pipeline/eval.py#L309-L332)

#### 功能特性

1. **错误驱动检索**：当代码执行失败时，自动提取错误信息作为查询关键词
2. **上下文增强**：结合完整代码上下文进行语义理解
3. **特殊场景优化**：对timm等常用库提供专门的处理逻辑
4. **缓存机制**：支持Prefect缓存以提升重复查询性能

#### 工作流程

```mermaid
sequenceDiagram
participant Agent as RD-Agent
participant Context7 as Context7服务
participant Search as 文档搜索引擎
Agent->>Context7 : 发送错误信息
Context7->>Context7 : 构建增强查询含代码上下文
Context7->>Search : 执行文档检索
Search-->>Context7 : 返回相关文档片段
Context7-->>Agent : 返回结构化文档结果
Agent->>Agent : 将文档信息附加到错误消息
```

**图表来源**
- [rdagent/components/agent/context7/__init__.py](file://rdagent/components/agent/context7/__init__.py#L25-L45)

## 应用场景

RD-Agent支持多个重要的数据驱动工业场景，每个场景都有专门的组件和优化策略。

### LLM微调场景

#### 数据科学任务微调

RD-Agent支持针对特定数据科学竞赛的微调流程，通过`DSFinetuneScen`类实现定制化配置。

```python
class DSFinetuneScen(DataScienceScen):
    """DSFinetuneScen 场景"""
    def _get_data_folder_description(self) -> str:
        folder_desc = describe_data_folder_v2(
            Path(DS_RD_SETTING.local_data_path) / self.competition,
            show_nan_columns=DS_RD_SETTING.show_nan_columns,
            max_length=20000,
        )
        return folder_desc
```

**章节来源**
- [rdagent/app/finetune/data_science/scen.py](file://rdagent/app/finetune/data_science/scen.py#L1-L21)
- [rdagent/app/finetune/data_science/conf.py](file://rdagent/app/finetune/data_science/conf.py#L1-L41)

#### LLM模型微调

支持对大型语言模型进行微调，通过`LLMFinetuneScen`类从Hugging Face Hub下载数据集并进行训练。

```python
class LLMFinetuneScen(DataScienceScen):
    """LLMFinetuneScen 场景"""
    def _download_data(self, competition: str):
        save_path = f"{DS_RD_SETTING.local_data_path}/{competition}"
        if Path(save_path).exists():
            logger.info(f"{save_path} already exists.")
        else:
            from huggingface_hub import snapshot_download
            snapshot_download(
                repo_id=competition,
                repo_type="dataset",
                local_dir=save_path,
                local_dir_use_symlinks=False,
            )
```

**章节来源**
- [rdagent/app/finetune/llm/scen.py](file://rdagent/app/finetune/llm/scen.py#L1-L88)
- [rdagent/app/finetune/llm/conf.py](file://rdagent/app/finetune/llm/conf.py#L1-L44)

### 量化金融场景

#### 自动化量化交易工厂

RD-Agent-Q是第一个数据驱动的多代理框架，用于自动化量化策略的全栈研发。

```mermaid
flowchart TD
Start([开始量化研发]) --> FactorGen[因子生成]
FactorGen --> FactorEval[因子评估]
FactorEval --> ModelGen[模型生成]
ModelGen --> ModelEval[模型评估]
ModelEval --> PortfolioBuild[组合构建]
PortfolioBuild --> PerformanceEval[绩效评估]
PerformanceEval --> Decision{是否满意?}
Decision --> |否| FactorGen
Decision --> |是| Optimize[优化策略]
Optimize --> End([结束])
```

**图表来源**
- [rdagent/app/qlib_rd_loop/quant.py](file://rdagent/app/qlib_rd_loop/quant.py#L20-L80)

#### 核心功能

1. **因子迭代进化**：自动提出和实现新的因子
2. **模型协同优化**：因子与模型的联合优化
3. **报告因子提取**：从财务报告中自动提取因子
4. **实时回测**：基于Qlib平台的实时策略回测

**章节来源**
- [rdagent/app/qlib_rd_loop/quant.py](file://rdagent/app/qlib_rd_loop/quant.py#L1-L144)

### 数据科学场景

#### 可配置代理选项增强

数据科学代理的配置选项得到显著增强，主要体现在`DataScienceBasePropSetting`类中。

```python
class DataScienceBasePropSetting(KaggleBasePropSetting):
    # MCP文档搜索集成
    enable_mcp_documentation_search: bool = False
    """启用MCP文档搜索以解决错误。需要MCP_ENABLED=true和MCP_CONTEXT7_ENABLED=true环境变量。"""

    # 模型转储控制
    enable_model_dump: bool = False
    model_dump_check_level: Literal["medium", "high"] = "medium"

    # 多轨迹控制
    max_trace_num: int = 1
    enable_cross_trace_diversity: bool = True
    diversity_injection_strategy: str = "rdagent.scenarios.data_science.proposal.exp_gen.diversity_strategy.InjectUntilSOTAGainedStrategy"
```

**章节来源**
- [rdagent/app/data_science/conf.py](file://rdagent/app/data_science/conf.py#L1-L207)

#### 支持的竞赛类型

1. **Kaggle竞赛**：支持各种类型的Kaggle比赛
2. **医疗预测**：医学数据的预测建模
3. **金融预测**：金融市场的时间序列预测
4. **通用数据科学**：各种数据科学挑战

**章节来源**
- [rdagent/scenarios/data_science/loop.py](file://rdagent/scenarios/data_science/loop.py#L1-L100)

### 研究助手场景

#### 自动化研究论文处理

RD-Agent可以自动阅读研究论文并实现模型结构或构建数据集。

```mermaid
sequenceDiagram
participant Researcher as 研究者
participant RD-Agent as RD-Agent
participant Paper as 研究论文
participant Code as 代码生成器
participant Executor as 执行器
Researcher->>RD-Agent : 提交论文URL
RD-Agent->>Paper : 下载并解析论文
Paper-->>RD-Agent : 返回文本内容
RD-Agent->>RD-Agent : 分析论文结构
RD-Agent->>Code : 生成实现代码
Code-->>RD-Agent : 返回代码
RD-Agent->>Executor : 执行代码
Executor-->>RD-Agent : 返回结果
RD-Agent-->>Researcher : 提供实现结果
```

**图表来源**
- [rdagent/app/cli.py](file://rdagent/app/cli.py#L40-L60)

**章节来源**
- [rdagent/app/cli.py](file://rdagent/app/cli.py#L1-L88)

## 创新特性

### 多轮迭代进化

RD-Agent的核心创新在于其多轮迭代进化机制，实现了真正的自主学习和改进。

#### 进化循环

```mermaid
flowchart LR
Idea[想法提案] --> Implement[实施执行]
Implement --> Evaluate[评估反馈]
Evaluate --> Learn[学习改进]
Learn --> Idea
subgraph "知识增强"
KnowledgeBase[知识库]
RAG[检索增强]
GraphDB[图数据库]
end
Learn --> KnowledgeBase
KnowledgeBase --> RAG
RAG --> GraphDB
GraphDB --> Idea
```

### 检索增强生成（RAG）

RD-Agent实现了先进的RAG策略，支持多种知识查询和融合方式：

1. **历史轨迹查询**：利用过去的成功和失败经验
2. **组件分析查询**：基于任务组件的相似性匹配
3. **错误分析查询**：从错误中学习和避免重复

### 知识图谱构建

项目构建了复杂的知识图谱，包含：

- **任务描述节点**：表示具体的任务要求
- **成功实现节点**：存储成功的代码实现
- **错误节点**：记录常见的错误模式
- **组件节点**：抽象的算法和方法组件

**章节来源**
- [rdagent/components/coder/CoSTEER/knowledge_management.py](file://rdagent/components/coder/CoSTEER/knowledge_management.py#L600-L964)

## 与其他工具的对比

### 传统自动化工具 vs RD-Agent

| 特性 | 传统自动化工具 | RD-Agent |
|------|---------------|----------|
| 学习能力 | 预定义规则，无自我改进 | 基于LLM的自主学习 |
| 知识管理 | 静态知识库 | 动态知识图谱 |
| 进化机制 | 固定流程 | 多轮迭代进化 |
| 应用场景 | 有限范围 | 多领域通用 |
| 智能程度 | 规则驱动 | 推理驱动 |

### 技术优势

1. **更强的泛化能力**：基于LLM的通用推理能力
2. **更好的适应性**：能够适应新的任务和环境
3. **更高的效率**：通过知识重用减少重复工作
4. **更好的质量**：通过多轮迭代获得更优解

### 性能表现

根据MLE-bench基准测试，RD-Agent在机器学习工程任务上表现出色：

| Agent | Low==Lite (%) | Medium (%) | High (%) | All (%) |
|-------|---------------|------------|----------|---------|
| R&D-Agent o3(R)+GPT-4.1(D) | 51.52 ± 6.9 | 19.3 ± 5.5 | 26.67 ± 0 | 30.22 ± 1.5 |
| R&D-Agent o1-preview | 48.18 ± 2.49 | 8.95 ± 2.36 | 18.67 ± 2.98 | 22.4 ± 1.1 |

**章节来源**
- [README.md](file://README.md#L50-L100)

## 总结

RD-Agent代表了人工智能在自动化研发领域的重大突破，通过以下核心创新实现了真正的智能自动化：

### 核心贡献

1. **架构创新**：提出了基于RAGEvoAgent和CoSTEER策略的新型自动化框架
2. **知识管理**：构建了先进的RAG知识管理系统和知识图谱
3. **场景覆盖**：支持数据科学、量化金融、Kaggle竞赛等多个重要场景
4. **性能领先**：在多个基准测试中取得优异成绩

### 技术特色

- **自主进化**：通过多轮迭代实现持续改进
- **知识增强**：利用检索增强生成提升性能
- **多模态支持**：支持文本、代码、数据等多种输入形式
- **实时反馈**：提供即时的执行反馈和改进建议

### 应用前景

RD-Agent为工业界提供了强大的自动化研发工具，能够显著提高研发效率和质量，降低技术门槛，推动人工智能在各个领域的深入应用。

随着技术的不断发展和完善，RD-Agent有望成为未来研发工作的重要基础设施，为人工智能时代的科技创新提供强有力的支持。