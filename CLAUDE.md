...
RD-Agent 是一个面向机器学习工程（MLE）的自主代理系统，旨在自动化研究和开发流程。该项目在 MLE-bench 基准测试中表现优异，专注于为数据科学竞赛、量化交易和通用机器学习任务提供智能化的实验开发和管理能力。
...
- **多场景支持**：Kaggle竞赛、量化交易、通用数据科学任务
- **进化式开发**：通过评估反馈循环不断改进代码和策略
- **知识管理**：自动积累和组织实验经验，支持RAG增强
- **多模态支持**：集成多种LLM后端（LiteLLM、OpenAI等）
